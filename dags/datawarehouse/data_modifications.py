...
def insert_rows(cur, conn, schema, row):
    """Function will take the cursor and connection objects, the schema to differentiate between
    staging and core, and a variable row, which is a dictionary containing the data to be inserted"""
    ...
    try:
        ...
        if schema == "staging":
            ...
            cur.execute(
                ...
                VALUES (%(video_ids)s, %(title)s, %(publishedAt)s, %(duration)s, %(viewCount)s, %(likeCount)s, %(commentCount)s);
                """, row
